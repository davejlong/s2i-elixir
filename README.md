...
The following command will create a builder image named elixir based on the Dockerfile that was created previously.
```
docker build -t elixir .
```
The builder image can also be created by using the *make* command since a *Makefile* is included.

Once the image has finished building, the command *s2i usage elixir* will print out the help info that was defined in the *usage* script.

#### Testing the builder image
The builder image can be tested using the following commands:
```
docker build -t elixir-candidate .
IMAGE_NAME=elixir-candidate test/run
```
